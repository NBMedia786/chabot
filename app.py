...
# ── NEW: CORS for cross-domain frontend on Hostinger
...
# ── NEW: Enable CORS for Hostinger → Render calls
CORS(app, resources={
    r"/*": {
...
def _get_eleven_token(participant_name: Optional[str] = None) -> str:
    ...
    # ── CHANGED: include dynamic_variables and user.name to satisfy agents that require them
    base_payload: Dict[str, object] = {"agent_id": AGENT_ID}
    if participant_name:
        base_payload["participant_name"]   = participant_name         # keep for legacy
        base_payload["dynamic_variables"]  = {"participant_name": participant_name}
        base_payload["user"]               = {"name": participant_name}
...
@app.get("/")
def index():
    # Safe to keep for local testing; Hostinger serves real frontend
...
# Conversation token — accepts BOTH ?participant_name= and ?name=
@app.get("/conversation-token")
def conversation_token():
    name = (request.args.get("participant_name") or request.args.get("name") or "").strip()
    cache_key = name or ""
...
# ===== Upload & blueprint (accepts form-data OR JSON now) =====
blueprint_storage = {}

@app.post("/upload-session")
def upload_session():
    # NEW: accept JSON or form-data for flexibility with static frontend
    if request.is_json:
        data = request.get_json(force=True) or {}
        email = (data.get("email") or "").strip()
        transcript = (data.get("markdown") or data.get("transcript") or "").strip()
    else:
    ...
    # Build absolute link safely behind proxy/CDN
    scheme = request.headers.get("X-Forwarded-Proto", request.scheme)
    host   = request.headers.get("X-Forwarded-Host", request.host)
    base   = f"{scheme}://{host}/"
    blueprint_link = f"{base}blueprint/{blueprint_id}"
    ...
    return jsonify({"ok": True, "scheduled_in_seconds": delay_seconds, "blueprint": blueprint_link})
