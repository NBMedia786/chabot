...
# Features:
# - ElevenLabs conversation token (with participant_name)
# - Supabase profiles upsert (name/email/phone/age)
# - Email delivery via SendGrid (preferred) with SMTP fallback
# - Gemini-powered mindmap blueprint generation + shareable link
# - CORS enabled for cross-origin frontend calls
# - Production-ready: use with gunicorn (see notes at bottom)
...
# Replace "https://your-frontend-domain.com" with your real Hostinger domain
CORS(app, resources={r"/*": {"origins": [
    os.getenv("FRONTEND_ORIGIN") or "https://ronit.nbmedia.co.in/",
    "http://localhost:3000",
    "http://localhost:5173",
...
@app.get("/")
def index():
    # Serve your frontend's index.html only if you ship it with the backend;
    # otherwise this can be removed (frontend is on Hostinger).
    return send_from_directory(str(ROOT_DIR), "index.html")

@app.get("/<path:path>")
def static_files(path):
    # Static files if you have any alongside app.py
    return send_from_directory(str(ROOT_DIR), path)
